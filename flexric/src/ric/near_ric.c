...
void control_service_near_ric(near_ric_t* ric, /*global_e2_node_id_t const* id,*/ uint16_t ran_func_id, const char* cmd)
{
  ...
  if(sm->ran_func_id == 142){
    ...
    printf("[NEAR-RIC]: MAC CONTROL SERVICE sent\n");

    e2ap_free_control_request_ric(&ric->ap, &ctrl_req);
    free_byte_array(ba_msg);
  } else if (sm->ran_func_id == 200){
    sm_ag_if_wr_t wr = {.type = MCS_CTRL_REQ_V0};
    wr.mcs_req_ctrl.hdr.dummy = 0;
    if(!strncmp(cmd,"ADD",5)){
      wr.mcs_req_ctrl.msg.add_mod_mcs = added_mod_mcs;
      ...
      printf("[NEAR-RIC]: MCS CONTROL SERVICE sent\n");

      e2ap_free_control_request_ric(&ric->ap, &ctrl_req);
      free_byte_array(ba_msg);

    } else if (!strncmp(cmd,"DEL",5)){
      assert(0!=0 && "Not implemented!");
    } else if (!strncmp(cmd,"ASSOC",5)){
      assert(0!=0 && "Not implemented!");
    } else{
      assert(0!=0 && "Unknown control type!");
    }
    
  } else{
    assert(0!=0 && "Unknown RAN function!" );
  }
