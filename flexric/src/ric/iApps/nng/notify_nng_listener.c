...
static
req_reply_server_arg_t arg_ctrl;
...
static
xapp_request_answer_e request_fun2(xapp_req_msg_t const* msg, void* data)
{
  assert(msg != NULL);
  assert(data != NULL);

// ToDO: Insert the code for managing the number of multiple control requests!
...
//  const char* url_pub = "ipc:///tmp/pubsub.ipc"; 
  ...
  const char* url_ctrl = "ws://0.0.0.0:9999";
...
//  const char* url_pub = "ws://192.168.52.10:9992";
  ...
  size_t const url_ctrl_sz = strlen(url_ctrl);

  arg_ctrl.init_fp = NULL;
  arg_ctrl.keep_alive_fp = NULL;
  arg_ctrl.request_fp = request_fun2;
  arg_ctrl.data = &reg;

  assert(url_ctrl_sz < sizeof(arg_ctrl.url)/sizeof(arg_ctrl.url[0]) && "Too large url for the struct req_rreply_server_arg_t");

  strncpy(arg_ctrl.url, url_ctrl, url_ctrl_sz); 

  init_req_reply_server_ctrl(&arg_ctrl);
...
void notify_nng_listener(sm_ag_if_rd_t const* data)
{
  ...
  if(data->type == MAC_STATS_V0){
    ...
    mcs_ind_msg_t const* ind = &data->mcs_stats.msg;

    char stats[2048] = {0};
    to_string_mcs(ind, ind->tstamp, stats, 2048);
    int rv;
    if ((rv = nng_send(sock, stats, strlen(stats) + 1, 0)) != 0) {
      fatal("nng_send", rv);
    }
