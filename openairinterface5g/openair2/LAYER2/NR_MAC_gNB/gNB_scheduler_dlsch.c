...
uint32_t* dl_mcs_set_value;
mcs_algorithm_e* dl_mcs_type;
pthread_t dl_scanthreadid;
uint32_t* dl_mcs_id;

void set_dl_mcs_type_and_value(){
  if(dl_mcs_value.mcs_conf.len_mcss > 0){
      dl_mcs_type = calloc(dl_mcs_value.mcs_conf.len_mcss, sizeof(mcs_algorithm_e));
      dl_mcs_set_value = calloc(dl_mcs_value.mcs_conf.len_mcss, sizeof(uint32_t));
      dl_mcs_id = calloc(dl_mcs_value.mcs_conf.len_mcss, sizeof(uint32_t));
      assert(dl_mcs_type != NULL);
      assert(dl_mcs_set_value != NULL);
      assert(dl_mcs_id != NULL);
  }

  printf("len_mcss: %d\n", dl_mcs_value.mcs_conf.len_mcss);
  for(size_t i = 0; i < dl_mcs_value.mcs_conf.len_mcss; ++i){
    printf("MCS id: %d\n",dl_mcs_value.mcs_conf.mcss[i].id);
    printf("MCS value: %d\n",dl_mcs_value.mcs_conf.mcss[i].sta.mcs_value);
    printf("MCS type: %d\n",dl_mcs_value.mcs_conf.mcss[i].type);
    dl_mcs_id[i] = dl_mcs_value.mcs_conf.mcss[i].id;
    if(dl_mcs_value.mcs_conf.mcss[i].type == MCS_ALG_SM_V0_STATIC){
      dl_mcs_type[i] = MCS_ALG_SM_V0_STATIC;
      dl_mcs_set_value[i] = dl_mcs_value.mcs_conf.mcss[i].sta.mcs_value;
      printf("DL MCS has been changed!\n");
    }
    else if (dl_mcs_value.mcs_conf.mcss[i].type == MCS_ALG_SM_V0_ORIGIN){
      dl_mcs_type[i] = MCS_ALG_SM_V0_ORIGIN;
    }
    else{
...
void* dl_scanThread(void *arg)
{
    printf("dl_scan thread created\n");
    int *dl_pNumber = (int *)arg;
    int lastNumber = *dl_pNumber;
    while(1)
    {
      while(lastNumber == *dl_pNumber ){
        pthread_mutex_lock(&dl_mcs_lock);
        pthread_cond_wait(&dl_mcs_has_changed,&dl_mcs_lock);
        pthread_mutex_unlock(&dl_mcs_lock);
      }
      printf("Changing DL MCS...\n");
      set_dl_mcs_type_and_value();
      lastNumber = *dl_pNumber;
    }

}

bool dl_mcs_control_initialized_flag = false;
...
void pf_dl(module_id_t module_id,
  ...
  size_t ue_i = 0;
  if(!dl_mcs_control_initialized_flag){
    pthread_create(&dl_scanthreadid,NULL,dl_scanThread,(void *)&dl_mcs_value.tstamp);
    dl_mcs_control_initialized_flag = true;
  }
  ...
  for (int UE_id = UE_list->head; UE_id >= 0; UE_id = UE_list->next[UE_id]) {
    ...
    if (sched_pdsch->dl_harq_pid >= 0) {
      ...
      if(dl_mcs_type == NULL || dl_mcs_type[ue_i] == MCS_ALG_SM_V0_ORIGIN){
        sched_pdsch->mcs = get_mcs_from_bler(module_id, /* CC_id = */ 0, frame, slot, UE_id);
        //LOG_I(NR_MAC, "Original MCS, current DL MCS value is %d\n", sched_pdsch->mcs);
      }else if (dl_mcs_type[ue_i] == MCS_ALG_SM_V0_STATIC){
        sched_pdsch->mcs = dl_mcs_set_value[ue_i];
        LOG_D(NR_MAC, "RIC works, current UE %04x DL MCS value is %d\n", UE_info->rnti[UE_id], sched_pdsch->mcs);
      }else{
        assert(0!=0 && "Unknown mcs type!");
      }
      ++ue_i;
