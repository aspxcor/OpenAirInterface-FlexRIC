...
uint32_t* ul_mcs_set_value;
mcs_algorithm_e* ul_mcs_type;
pthread_t ul_scanthreadid;
uint32_t* ul_mcs_id;

void set_ul_mcs_type_and_value(){
  if(ul_mcs_value.mcs_conf.len_mcss > 0){
      ul_mcs_type = calloc(ul_mcs_value.mcs_conf.len_mcss, sizeof(mcs_algorithm_e));
      ul_mcs_set_value = calloc(ul_mcs_value.mcs_conf.len_mcss, sizeof(uint32_t));
      ul_mcs_id = calloc(ul_mcs_value.mcs_conf.len_mcss, sizeof(uint32_t));
      assert(ul_mcs_type != NULL);
      assert(ul_mcs_set_value != NULL);
      assert(ul_mcs_id != NULL);
  }

  for(size_t i = 0; i < ul_mcs_value.mcs_conf.len_mcss; ++i){
    ul_mcs_id[i] = ul_mcs_value.mcs_conf.mcss[i].id;
    if(ul_mcs_value.mcs_conf.mcss[i].type == MCS_ALG_SM_V0_STATIC){
    ul_mcs_type[i] = MCS_ALG_SM_V0_STATIC;
    ul_mcs_set_value[i] = ul_mcs_value.mcs_conf.mcss[i].sta.mcs_value;
    printf("UL MCS has been changed!\n");
    }
    else if (ul_mcs_value.mcs_conf.mcss[i].type == MCS_ALG_SM_V0_ORIGIN){
      ul_mcs_type[i] = MCS_ALG_SM_V0_ORIGIN;
    }
    else{
...
void* ul_scanThread(void *arg)
{
    printf("ul_scan thread created\n");
    int *ul_pNumber = (int *)arg;
    int lastNumber = *ul_pNumber;
    while(1)
    {
      while(lastNumber == *ul_pNumber ){
        pthread_mutex_lock(&ul_mcs_lock);
        pthread_cond_wait(&ul_mcs_has_changed,&ul_mcs_lock);
        pthread_mutex_unlock(&ul_mcs_lock);
      }
      printf("Changing UL MCS...\n");
      set_ul_mcs_type_and_value();
      lastNumber = *ul_pNumber;
    }

}

bool ul_mcs_control_initialized_flag = false;
...
void pf_ul(module_id_t module_id,
  ...
  size_t ue_i = 0;
  if(!ul_mcs_control_initialized_flag){
    pthread_create(&ul_scanthreadid,NULL,ul_scanThread,(void *)&ul_mcs_value.tstamp);
    ul_mcs_control_initialized_flag = true;
  }
  ...
  for (int UE_id = UE_list->head; UE_id >= 0; UE_id = UE_list->next[UE_id]) {
    ...
    if (B == 0 && do_sched) {
      ...
      if(ul_mcs_type == NULL || ul_mcs_type[ue_i] == MCS_ALG_SM_V0_ORIGIN){
        sched_pusch->mcs = 9;
        //LOG_I(NR_MAC, "Original MCS, current UL MCS value is %d\n", sched_pusch->mcs);
      }else if (ul_mcs_type[ue_i] == MCS_ALG_SM_V0_STATIC){
        sched_pusch->mcs = ul_mcs_set_value[ue_i];
        LOG_I(NR_MAC, "RIC works, current UE %04x UL MCS value is %d\n", UE_info->rnti[UE_id], sched_pusch->mcs);
      }else{
